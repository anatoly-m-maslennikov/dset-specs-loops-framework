...
2. Identify the one current owner and classify the requested content using the [artifact-type catalog](artifact-types.md).
...
6. Update `dset/scopes/gov/artifacts.yaml` when a governed area, hub, owner, purpose, or parent changes.
7. Apply the [hub rules](hub-rules.md) and update the parent hub only when the stable navigation route changes; do not add every leaf file.
