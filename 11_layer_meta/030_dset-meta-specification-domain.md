...
|---|---|---|
| **Revision mode** | How a persisted project item may change: `atomic` is accepted and immutable; `evergreen` is a mutable current view; `maintained` is a mutable operative item | `DSET-REQUIREMENT-META-035` |
...
- **DSET-INVARIANT-META-016:** Non-derived provenance, priority, applicability, relation endpoints, and type-specific facts remain explicit. Acceptance is inherent to emitted atoms, active versus archived state comes from repository placement, and ambient project authority is not repeated. Sources: `DSET-REQUIREMENT-META-035` and `DSET-REQUIREMENT-GOV-094`.
