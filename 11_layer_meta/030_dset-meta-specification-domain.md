...
|---|---|---|
| **Revision mode** | How a persisted project item may change: `atomic` is one independently governed immutable unit; `append_only` preserves accepted records and permits only complete new records; `maintained` permits governed revision of existing content | `DSET-REQUIREMENT-META-040` |
...
| **Artifact type** | One canonical persisted classifier whose optional direct subtype and registered mapping resolve exactly one Artifact route | `DSET-REQUIREMENT-META-013`, `DSET-REQUIREMENT-META-035` |
...
| **Append-only sequence** | A Governed artifact whose accepted records remain immutable and ordered while complete new records may be appended | `DSET-REQUIREMENT-META-040` |
...
| **Evergreen document** | A thin, reasoned, maintained Governed artifact with an explicit freshness obligation that presents current meaning while returning every summarized claim directly to Atomic records | `DSET-REQUIREMENT-META-020`, `DSET-REQUIREMENT-META-033`, `DSET-REQUIREMENT-META-040` |
| **Governance surface** | A named optional maintained Governed artifact whose activation creates currentness or gate obligations | `DSET-REQUIREMENT-META-033`, `DSET-REQUIREMENT-META-040` |
...
| **Test plan** | A maintained Method with an evergreen freshness obligation that organizes deterministic check obligations and their Atomic records | `DSET-REQUIREMENT-META-001`, `DSET-REQUIREMENT-META-002`, `DSET-REQUIREMENT-META-007`, `DSET-REQUIREMENT-META-040` |
| **Evaluation plan** | A maintained Method with an evergreen freshness obligation that organizes qualitative, probabilistic, statistical, or model-judged obligations and their Atomic records | `DSET-REQUIREMENT-META-001`, `DSET-REQUIREMENT-META-002`, `DSET-REQUIREMENT-META-007`, `DSET-REQUIREMENT-META-040` |
...
- **DSET-INVARIANT-META-012:** Generated Code and hand-maintained executable truth are maintained Implementations with different update procedures; generated output additionally declares generator and source provenance. A Git commit is a separate atomic Implementation record. Source: `DSET-REQUIREMENT-META-040`.
...
- **DSET-INVARIANT-META-033:** DSET starts atomic-first; named maintained governance surfaces, including surfaces with evergreen freshness obligations, are optional and may be activated or deactivated without changing atomic authority. Sources: `DSET-REQUIREMENT-META-033` and `DSET-REQUIREMENT-META-040`.
...
- **DSET-INVARIANT-META-035:** Every governed artifact resolves exactly one Revision mode: `atomic`, `append_only`, or `maintained`. Evergreen freshness and artifact-specific update mechanisms remain separate concerns. Source: `DSET-REQUIREMENT-META-040`.
