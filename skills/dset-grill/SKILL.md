...
This is the thin wrapper for the registered `domain-grilling` workflow. Repository-local governing documents own every substantive rule.

## Bootstrap and invocation

1. Locate the repository root by walking upward to `dset/dset.yaml`.
2. Run `dset rules resolve domain-grilling --format json` or `python -m dset_toolchain rules resolve domain-grilling --format json`.
