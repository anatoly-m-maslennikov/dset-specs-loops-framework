---
name: dset-decide
description: Create a durable immutable DSET Decision from a consequential resolved Question and sufficient current evidence. Use after clarification and landscape work make the choice decision-ready; compile the authorized consequence handoff but stop before implementation.
---

# DSET Decide

This is the thin wrapper for `decide`; resolved repository-local documents own substantive behavior.
...
3. Run the selected entrypoint with `rules resolve decide --format json`; stop on a nonzero result without fallback to the wrapper, memory, installed templates, or remote framework prose.
...
Return the emitted Decision identity, rationale/evidence links, replaced or absorbed claim links, evergreen compilation obligations, and available session identity. Never edit an emitted atom; stop before implementation.
