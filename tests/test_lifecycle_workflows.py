...
class LifecycleWorkflowTests(unittest.TestCase):
    def test_every_mode_has_a_registered_or_exception_route(self) -> None:
        ...
        self.assertEqual(
            modes,
            EXCEPTION_MODES | set(SPECIALIST_WORKFLOWS) | INTERNAL_WORKFLOWS,
        )
        self.assertTrue(INTERNAL_WORKFLOWS.issubset(registered))
        self.assertTrue(set(SPECIALIST_WORKFLOWS.values()).issubset(registered))

    def test_internal_modes_resolve_without_public_wrappers(self) -> None:
        registry = cast(dict[str, Any], load(REGISTRY))
        ...
        self.assertTrue(INTERNAL_WORKFLOWS.isdisjoint(wrapped))
        self.assertNotIn("work-triage", wrapped)

        for workflow_id in sorted(INTERNAL_WORKFLOWS):
            with self.subTest(workflow=workflow_id):
                ...
                self.assertIsNone(resolved["wrapper"])
    ...
    def test_distributed_profile_matches_repository_workflows(self) -> None:
        ...
        self.assertEqual(len(profile_wrappers), 5)
