...
Before copied or substantially adapted third-party material enters this repository, record its exact source and revision, license, affected files, modifications, required notices, and trademark boundary here and in [`dset/project/provenance.toml`](dset/project/provenance.toml). Add the applicable license text under `third_party/licenses/`. Independently re-expressed ideas must still cite their source when it materially shaped the design; web-only references may be recorded here with their observation date when no versioned source artifact is imported.
