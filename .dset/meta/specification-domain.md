...
| **User Story** | A direct Decision subtype naming an actor or stakeholder, desired capability or outcome, and value or purpose without absorbing linked acceptance criteria; its ID token remains `STORY` for compatibility |
| **Outcome** | A direct Decision subtype defining an intended measurable change in user, business, operational, or system state with baseline, target, observation method/source, and evaluation window where applicable; the observed result is evidence |
| **Requirement** | A direct Decision subtype defining the residual observable, verifiable behavior, result, capability, quality, or prevention condition no more precise Decision subtype owns |
...
| **Scenario** | A direct Decision subtype defining a concrete accepted behavior or edge case through preconditions, interaction or event, and expected observable result; the executed run is work/evidence |
| **Invariant** | A direct Decision subtype defining a condition that must always hold in its declared scope |
