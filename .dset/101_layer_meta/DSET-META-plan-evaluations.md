...
28. A repository-level project and monorepos with one or many local, deployable, library, documentation, methodology, data, and mixed-content Work Areas, including a resumed session carrying a stale scope hint.
29. Mixed directive, acceptance-event, carrier, execution, result, and evidence records; multi-head Decision claims; boundary-reliant restrictions; observed versus intended Outcomes; Scenario definitions versus runs; and deterministic code that executes judgment-based Evaluation.

| Eval ID | Criterion | Threshold |
|---|---|---|
| **DSET-EVAL-PLAN-META-001** | Stage routing | Independent reviewers select the same owning stage/document for all eight scenarios |
| **DSET-EVAL-PLAN-META-002** | Test/eval separation | No reviewer classifies deterministic correctness as an eval merely because it is automated |
| **DSET-EVAL-PLAN-META-003** | Runtime applicability | Reviewers select only the recovery, effect-safety, and durability mechanisms triggered by each scenario |
| **DSET-EVAL-PLAN-META-004** | Implementation readiness | No scenario leaves a critical ownership, proof, or recovery decision implicit |
| **DSET-EVAL-PLAN-META-005** | Profile independence | Reviewers configure documentation-only, Python-plus-documentation, and future TypeScript-plus-documentation cases without treating documentation as a programming language |
| **DSET-EVAL-PLAN-META-006** | Contract restraint | Every reviewer classifies an operator-accepted boundary as Decision/Contract, a solution restriction as Decision/Constraint, ambiguity as Question, incompatible active authority as Question/Conflict, and observed nonconformance as Problem; requires explicit precedence or an absorbing Contract for change; and rejects prose-only host/platform/CI assurance |
| **DSET-EVAL-PLAN-META-007** | Flat Decision-subtype clarity | Every reviewer treats User Story, Requirement, Constraint, Contract, Outcome, Scenario, and Invariant as sibling Decision subtypes; selects the subtype whose acceptance condition owns the primary claim; splits acceptance criteria and other multi-head claims; distinguishes atoms from acceptance acts, carriers, work, and evidence; never emits Decision/Requirement/User Story nesting; and omits User Story when no meaningful actor perspective exists |
| **DSET-EVAL-PLAN-META-008** | Outcome validity | Every reviewer treats Outcome as a direct Decision subtype for intended state change, distinguishes delivered output and observed evidence from that directive, rejects unusable baseline/target/method/window claims, and keeps sibling Requirement-delivery evidence separate from QA/Evaluation evidence of intended effect |
| **DSET-EVAL-PLAN-META-009** | Work Area neutrality and scope clarity | Every reviewer preserves repository-level and one-or-many Work Area options, uses repository-relative folder boundaries without inferring code, deployability, services, features, or modules, preserves mixed-content areas without splitting or classifying them artificially, and treats a session's Work Area reference as non-authoritative until it is re-resolved |
