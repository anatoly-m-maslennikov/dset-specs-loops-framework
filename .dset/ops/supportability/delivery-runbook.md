...
| DSET relationship view | `.dset/project/generated/traceability.toml` | change ID and PR URL; GitHub remains authoritative |
...
The repository now contains the guarded `Publish DSET release` workflow and deterministic local `dset release plan`, `check`, and explicit `prepare --execute` transaction. The publisher runs only after a protected `main` push that changed `.dset/versions/version.toml`; it checks the committed declaration, verifies the checkout SHA, creates only missing GitHub objects, and stops on tag identity or SHA collisions without a post-merge content write.
