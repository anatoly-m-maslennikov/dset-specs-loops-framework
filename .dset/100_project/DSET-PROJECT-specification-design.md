...
|---|---|---|---|---|
| Selected profile | `.dset/dset_settings.toml` | Project change | Explicit reviewed edit | Schema and repository check |
...
| Released baseline | Commit in `10_versions/version.toml` | Release process | Candidate self-host run | Extracted released-validator check |
...
| Product/package release identity | Change release declaration; `10_versions/version.toml` and package metadata are mirrors | Reviewed integration-to-protected release PR | Exactly once per accepted PR | Version transition and surface-consistency tests |
...
- Expose the direct subtype kind in an atom ID when a subtype exists and the Type kind when it does not; never encode a nested subtype path. A naming-vocabulary change performs one complete migration of current and historical IDs, filenames, references, lifecycle targets, and generated surfaces. Preserve history through provenance and lineage, not accepted aliases.

## Open questions

- **DSET-QUESTION-GOV-001 — First-class Action entity** in `intake.toml`:
...
- Six additional open Questions in `intake.toml` defer
