...
**Scenario DSET-SCENARIO-GOV-005:** `.dset/project/README.md` exposes accepted truth, active changes, archive history, templates, and schemas, while no committed project truth is owned by `.dset/`.
...
The append-only `.dset/project/migrations/carrier-transitions.toml` ledger
...
`.dset/layer_1_meta/`, `.dset/layer_2_gov/`, `.dset/layer_3_tool/`, `.dset/layer_4_skill/`, and `.dset/layer_5_ops/`.
...
The five current filesystem carriers are ordered as `layer_1_meta`,
`layer_2_gov`, `layer_3_tool`, `layer_4_skill`, and `layer_5_ops`. The logical
