...
| Rule routing/provenance | `.dset/project/governance.toml` | Explicit materialize/refresh transaction | Every governed invocation | Governance schema and stable diagnostics |
...
| Released baseline | Commit in `.dset/versions/version.toml` | Release process | Candidate self-host run | Extracted released-validator check |
...
- **DSET-QUESTION-GOV-001 — First-class Action entity** in [`.dset/project/intake.toml`](intake.toml):
...
- Six additional open Questions in [`.dset/project/intake.toml`](intake.toml) defer
