...
**Scenario DSET-SCENARIO-GOV-005:** `00_project/README.md` exposes accepted truth, active changes, archive history, templates, and schemas, while no committed project truth is owned by `.dset/`.
...
The append-only `00_project/migrations/carrier-transitions.toml` ledger
...
registries, migrations, evidence, verification, and generated views live under
`00_project/`; Version lifecycle artifacts, Changes, archives, and delivery
history live under `10_versions/`; and layer-owned truth lives directly under
`.dset/01_layer_meta/`, `.dset/02_layer_gov/`, `.dset/03_layer_tool/`, `.dset/04_layer_skill/`, and `.dset/05_layer_ops/`.
...
may instead be relative to the Markdown file that owns the link.
...
Schema 1.0–1.2 layouts and root settings are migration inputs only. New
...
## DSET-REQUIREMENT-GOV-044 — Use visibly ordered layer directories

The five current filesystem carriers are ordered as `01_layer_meta`,
`02_layer_gov`, `03_layer_tool`, `04_layer_skill`, and `05_layer_ops`. The logical
layer vocabulary remains `meta`, `gov`, `tool`, `skill`, and `ops` in semantic
IDs, package metadata, scopes, relations, public APIs, and operator commands.
The numeric prefix is a navigation affordance only.

Schema 1.3 selects the current topology with
`[structure].layout = "numbered-layers-v1"`. The earlier `slim-v1` directories
remain explicit read-compatible migration inputs, never competing writable
owners. Initialization emits numbered roots. Migration records byte-exact
immutable carrier moves and Git return addresses, rewrites mutable current
paths and links, refreshes generated views, and rejects mixed old/new roots.
