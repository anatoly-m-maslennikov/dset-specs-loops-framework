...
from dset_toolchain.layout import RepositoryLayout, discover_layout
...
class YamlSubsetTests(unittest.TestCase):
    def test_parses_repository_manifests(self) -> None:
        ...
        cases = load(unique_carrier(ROOT, "020_dset-tool-fixtures-cases.toml"))
        self.assertEqual(
            provenance["source_provenance"]["project_license"],
            "Apache-2.0",
        )
    ...
    def test_toml_array_tables_in_frontmatter_are_not_wiki_links(self) -> None:
        with temporary_directory() as raw:
            ...
            self.assertEqual(_validate_markdown(root, layout), [])
