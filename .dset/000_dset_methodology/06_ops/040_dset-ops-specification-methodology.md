...
IMPL owns the concrete environment, production and QA code, code-focused gates,
diagnostic emission, safe diagnostic interfaces, and rollback hooks. When OPS
finds that surface missing or insufficient, it emits a Problem, Question,
Requirement, or Decision for the appropriate upstream owner and waits for a
later implementation. OPS never directly governs or mutates IMPL.

## DSET-REQUIREMENT-OPS-001 — Production supportability is explicit and risk-scaled

Before a production-bound implementation is accepted for delivery, OPS must
confirm a supportability contract appropriate to its runtime risk profile and
deployment topology. The operational contract owns incident triggers or
objectives; evidence use; correlation and deploy/change identity; permissions;
retention, redaction, access, deletion, volume, cardinality, and sampling
bounds; runbook, escalation, rollback or kill-switch operation; and
incident-to-requirement/change/PR/fix traceability. IMPL owns the code that
realizes the required surface. Telemetry is diagnostic evidence, not a
competing business-state authority. A non-production tool may mark the OPS
gate not applicable only with a reason.
