...
| Entity | Definition |
|---|---|
| **Production supportability contract** | Risk- and topology-scaled post-implementation agreement for operating evidence, correlation and deploy/change identity, access and data controls, runbook, recovery/escalation paths, and incident-to-fix traceability; IMPL separately owns the code that realizes the required surface |
...
- **DSET-INVARIANT-OPS-001:** Every production-bound implementation passes an explicit supportability gate scaled to its risk profile and topology; a non-production tool may mark it not applicable only with a reason, and an implementation deficiency is routed upstream rather than prescribed backwards by OPS.
