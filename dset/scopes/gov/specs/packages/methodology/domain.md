...
- **DSET-INVARIANT-GOV-019:** Every governed artifact has exactly one primary artifact type and at most one allowed direct artifact subtype, independently from semantic Type. Analysis, specification, procedure, plan, implementation, evidence, verification, gate, publication, derived-view, and navigation roles remain distinct; unknown, missing, nested, mismatched, or multiply resolved classifications fail closed.
...
- **DSET-INVARIANT-GOV-023:** Artifact inheritance uses only canonical `child_of` links and derived reverse `parent_to` links. In a target scope, the nearest parent applies directly when no child exists; an accepted local child may add an implementation Decision, replace, or cancel the parent only for that scope and its descendants. Parents remain immutable, sibling scopes remain unaffected, and incompatible same-scope children stop.
