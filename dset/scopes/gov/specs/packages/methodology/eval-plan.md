...
| **DSET-EVAL-GOV-012** | Source-to-projection clarity | Reviewers distinguish atomic authority sources, evergreen compiled projections, transactional context/evidence, and implementation; select an active atom over a stale projection; preserve atom immutability; and reject implementation-only change that skips compilation |
...
| **DSET-EVAL-GOV-016** | Priority and conflict clarity | Reviewers identify effective priority for every governed artifact; classify Requirement/Contract, Contract/Contract, active-atom/compiled-projection, absorbing/absorbed, authority/Test-or-Eval, implementation/authority, evidence/evidence, and generated/canonical conflicts; select a claim only for permitted policy conflicts; avoid false compliance, evidence-as-authority, and age-based precedence; stop when required; and explain handling order from priority plus dependencies/gates |
| **DSET-EVAL-GOV-017** | Problem/Question/Conflict clarity | Reviewers consistently distinguish a harmful state from missing knowledge or choice and from verified incompatible applicable claims; avoid emitting Conflicts for ordinary drift, failed assurance, nonconformance, or wording differences; and link the smallest necessary Problem, Question, Decision, authority update, or lifecycle event without editing the Conflict atom |
