...
|---|---|---|---|---|---|---|
| Normative rules embedded in each skill/current pattern | Repository/project license | Registry-owned rules / ownership and maintenance | [Wrapper inventory](proofs/wrapper-rule-inventory-2026-07-14.md); current for recorded skill sources | sufficient | Unfavorable: duplicates authority and increases drift | Duplicate maintenance; observed wrapper/rule drift risk |
| Remote framework documents as live authority/current public repo | Public repository/project license | Repository-owned rules / offline and customized operation | [Architecture rule](../../../gov/governance/architecture.md); current for the authority Contract | sufficient | Unfavorable: fails local ownership and offline use | Network/runtime dependency; local customization cannot own truth |
| Generated local copies with silent framework fallback/custom | Project license | Explicit local registry / precedence and reproducibility | [Lifecycle failure behavior](../../governance/lifecycle-orchestration.md); current for fail-closed resolution | sufficient | Not eligible under the fixed no-hidden-fallback constraint | Hidden precedence and non-reproducible choices |
| Registry-resolved local governing documents plus thin wrappers/current implementation | Repository/project license | Embedded/remote/fallback models / authority, offline use, customization | [Local gate](proofs/local-gate-2026-07-15.md); current locally; hosted proof is stale | degraded | Favorable locally for §§0–§4; hosted comparison remains pending | Registry/materialization maintenance; distribution remains open |
| One public skill per lifecycle step/earlier inventory | Repository/project license | Five-skill topology / discovery and trigger overlap | [Skill-surface specification](specs/methodology.md#added--dset-requirement-skill-004-public-skills-remain-small-and-purposeful); runtime eval pending | insufficient | Inconclusive until host-native usability evals | High discovery and maintenance cost; overlapping triggers |
| One universal skill with embedded lifecycle rules/custom | Repository/project license | Thin orchestrator / single-owner authority | [Architecture rule](../../../gov/governance/architecture.md); current for rule ownership | sufficient | Not eligible under the fixed repository-owned-rule constraint | One oversized competing rule store |
| Primary orchestrator plus clarify/diagnose/prototype/release specialists/current proposal | Repository/project license | One-skill and per-step alternatives / discovery, ownership, stop boundaries | [Skill-surface specification](specs/methodology.md#added--dset-requirement-skill-004-public-skills-remain-small-and-purposeful); implementation incomplete | degraded | Favorable on specified topology; runtime comparison pending | Small discovery surface; two wrappers and host distribution remain |
| Decimal version arithmetic (`+0.1`/`+0.01`) | Informal convention/no imported code | Integer SemVer tuple / transition correctness | [Release transition table](../../../ops/governance/release.md#classification-and-transitions); current for specified edges | sufficient | Unfavorable: arithmetic can claim 1.0 without readiness | Low implementation cost; invalid `0.9 + 0.1` promotion |
| SemVer-compatible tuple with DSET normal/small classes/SemVer 2.0.0 | [SemVer 2.0.0](https://semver.org/), CC BY 3.0 reference | Decimal arithmetic / ecosystem mapping and RC ordering | [Release transition table](../../../ops/governance/release.md#classification-and-transitions); automation pending | degraded | Favorable for specification; execution proof pending | Transition and mirror-validation cost |
| Always lowest token-price model/generic heuristic | No governed source | Same-model inheritance / completed-task cost | [Delegation evidence rule](../../governance/delegation-budget.md#model-selection-evidence); no task-relevant DSET comparator | insufficient | Evidence-needed; nominal price alone is not comparable completed-task cost | Retry/token expansion can cost more and fail more often |
| Same model/effort by default; vary useful fan-out and evidence/current policy | DSET policy; [DeepSWE v1.1](https://deepswe.datacurve.ai/), observed 2026-07-15 | Price-only downgrade / outcome quality and completed-task cost | [Delegation evidence rule](../../governance/delegation-budget.md#model-selection-evidence); benchmark supports only the token-price limitation | degraded | Provisionally favorable; DSET fan-out comparison remains pending | Inheritance may use expensive models; avoids unsupported downgrade risk |

## Decision

Selection and its lifecycle are recorded separately in
[`DSET-DECISION-SKILL-001`](decision-DSET-DECISION-SKILL-001.md). Candidate
rows above contain comparison results only; they do not authorize adoption,
implementation, release, or rejection.
