...
| `DSET-EVAL-SKILL-008` | A public skill chains internal workflows, the host compacts context, and one stale plus one current authority signal exist at resume | Session recovery usefulness | Every operator resumes the intended unambiguous session, understands completed/pending work and authorization state, follows newer authoritative evidence over stale checkpoint hints, and never asks for a sixth public skill |
...
| `DSET-EVAL-META-009` | A repository-level project and monorepos with one or many local, deployable, library, documentation, methodology, data, and mixed-content Work Areas, including a resumed session carrying a stale scope hint | Work Area neutrality and scope clarity | Every reviewer preserves repository-level and one-or-many Work Area options, uses repository-relative folder boundaries without inferring code, deployability, services, features, or modules, preserves mixed-content areas without splitting or classifying them artificially, and treats a session's Work Area reference as non-authoritative until it is re-resolved |
| `DSET-EVAL-OPS-009` | Projects using the default shared integration branch and optional isolated worktrees, including concurrent Changes and one protected release PR | Delivery-topology clarity | Every reviewer identifies local integration, remote integration, protected target, actual Change workspace mode, and proof owner; none requires a worktree by default, merges Change identities because a branch or PR is shared, or creates permanent layer branches |
