...
| `DSET-EVAL-META-009` | A repository-level project and monorepos with one or many local, deployable, library, documentation, methodology, data, and mixed-content Work Areas, including a resumed session carrying a stale scope hint | Work Area neutrality and scope clarity | Every reviewer preserves repository-level and one-or-many Work Area options, uses repository-relative folder boundaries without inferring code, deployability, services, features, or modules, classifies mixed-content areas without splitting them artificially, and treats a session's Work Area reference as non-authoritative until it is re-resolved |
