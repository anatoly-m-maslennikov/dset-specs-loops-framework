...
|---|---|---|---|---|
| Selected profile | `dset/scopes/meta/dset.toml` | Project change | Explicit reviewed edit | Schema and repository check |
| Rule routing/provenance | `dset/scopes/gov/governance.toml` | Explicit materialize/refresh transaction | Every governed invocation | Governance schema and stable diagnostics |
...
| Released baseline | Commit in `dset/scopes/meta/version.toml` | Release process | Candidate self-host run | Extracted released-validator check |
...
| Product/package release identity | Change release declaration; `dset/scopes/meta/version.toml` and package metadata are mirrors | Reviewed integration-to-protected release PR | Exactly once per accepted PR | Version transition and surface-consistency tests |
...
- **DSET-QUESTION-GOV-001 — First-class Action entity** in [`dset/scopes/gov/intake.toml`](../../../gov/intake.toml):
...
- Six additional open Questions in [`dset/scopes/gov/intake.toml`](../../../gov/intake.toml) defer
