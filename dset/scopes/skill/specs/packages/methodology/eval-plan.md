...
| **DSET-EVAL-SKILL-003** | Public routing usefulness | Every reviewer selects the same stable mode for each lifecycle/authority-conflict state, chooses `dset` when the next mode is unknown, chooses the matching direct wrapper when the mode is explicit, and respects workflow and authorization stops |
