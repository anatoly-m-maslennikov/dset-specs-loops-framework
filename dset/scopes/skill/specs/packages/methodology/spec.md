...
The five public skills must share one internal session-continuity capability rather than expose another user-facing skill. `dset` starts, checkpoints, and resumes its workflow chain; a directly invoked specialist starts or joins a compatible session; automatically chained skills, governed model-only workflows, and delegated work remain child runs. Every run carries session/root/parent identity, and the runtime atomically updates a bounded ignored `.dset/sessions/<session-id>.json` checkpoint after workflow transitions, before an observable context handoff or compaction, and on terminal exit.
