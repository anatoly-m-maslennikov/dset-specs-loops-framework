...
- **DSET-INVARIANT-SKILL-003:** The same project-agnostic skill and canonical workflow-wrapper hash may serve multiple projects, but every invocation discovers exactly one root from its target and resolves only that project's local governance; changing or switching local rules changes the next governed invocation without changing the wrapper or leaking policy between projects.
- **DSET-INVARIANT-SKILL-004:** The release-target user-facing surface contains one primary `dset` orchestrator plus one explicit thin direct-entry wrapper for every stable lifecycle mode. Governed wrappers resolve exactly one registered repository-local workflow; initialization and governance repair remain the only bounded pre-resolution exceptions. Direct wrappers never create a second substantive rule owner.
...
- **DSET-INVARIANT-SKILL-009:** Public skills accept desired outcomes and may satisfy missing entry criteria only through a finite registered prerequisite closure; every transition makes progress and preserves session provenance, while no progress, repeated state, cycles, ambiguity, failure, or new authorization stops the chain.
- **DSET-INVARIANT-SKILL-010:** Implementation preparation mode changes only whether `dset-implement` closes missing prerequisite artifacts. Lazy mode uses the finite governed closure; strict mode invokes implementation only and stops on insufficient input. Neither mode weakens local governance, authority, authorization, provenance, Verification, or release boundaries.
