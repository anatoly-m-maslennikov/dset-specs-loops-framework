...
Fixtures use two complete base changes plus deterministic case mutations from [`cases.toml`](cases.toml). The test runner materializes each case in a temporary directory, applies only the declared mutations, and checks the expected stable diagnostic code.
