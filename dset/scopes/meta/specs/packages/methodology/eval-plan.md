...
| **DSET-EVAL-META-007** | Flat Decision-subtype clarity | Every reviewer treats User Story, Requirement, Constraint, Contract, Outcome, Scenario, and Invariant as sibling Decision subtypes; selects the subtype whose acceptance condition owns the primary claim; splits acceptance criteria and other multi-head claims; distinguishes atoms from acceptance acts, carriers, work, and evidence; never emits Decision/Requirement/User Story nesting; and omits User Story when no meaningful actor perspective exists |
...
| **DSET-EVAL-META-009** | Work Area neutrality and scope clarity | Every reviewer preserves repository-level and one-or-many Work Area options, uses repository-relative folder boundaries without inferring code, deployability, services, features, or modules, preserves mixed-content areas without splitting or classifying them artificially, and treats a session's Work Area reference as non-authoritative until it is re-resolved |
