...
| **DSET-EVAL-META-007** | Flat Decision-subtype clarity | Every reviewer treats User Story, Requirement, Constraint, Contract, Outcome, Scenario, and Invariant as sibling Decision subtypes; distinguishes their semantics from Design and plan roles; never emits Decision/Requirement/User Story nesting; and omits User Story when no meaningful actor perspective exists |
| **DSET-EVAL-META-008** | Outcome validity | Every reviewer treats Outcome as a direct Decision subtype, distinguishes a delivered output from measurable state change, rejects unusable baseline/target/method/window claims, and keeps sibling Requirement-delivery evidence separate from QA/Evaluation evidence of intended effect |
