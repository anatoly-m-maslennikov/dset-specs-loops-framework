...
| **Contract** | An immutable versioned non-choosable authoritative boundary, such as an existing DDL, required CSV/XLSX schema, supplied OpenAPI/message/protocol, host package format, platform matrix, hosted-CI interface, or dependency policy; implementation conforms to it, and only its named authority may emit an absorbing successor or retirement event |
...
- **DSET-INVARIANT-META-005:** An emitted Contract atom is immutable and remains the authoritative implementation boundary while active. Only its named authority may emit an absorbing successor or retirement event; current state and reverse links are derived. Implementation and Decisions cannot rewrite it: ambiguity becomes a Question, incompatibility becomes a Problem, and lifecycle is limited to append-only `declared -> active -> absorbed` or `declared -> active -> retired` events.
