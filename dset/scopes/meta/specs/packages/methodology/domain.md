...
| **User Story** | A direct Decision subtype naming an actor or stakeholder, desired capability or outcome, and value or purpose; its ID token remains `STORY` for compatibility |
| **Outcome** | A direct Decision subtype defining a measurable change in user, business, operational, or system state with baseline, target, observation method/source, and evaluation window where applicable |
| **Requirement** | A direct Decision subtype defining observable, verifiable behavior, result, capability, quality, or outcome the project must provide or prevent |
...
| **Scenario** | A direct Decision subtype defining a concrete accepted behavior or edge case through preconditions, interaction or event, and observable result |
...
- **DSET-INVARIANT-META-006:** User Story, Requirement, Constraint, Contract, Outcome, Scenario, and Invariant are sibling Decision subtypes. Links between them never create a subtype path. User Story owns actor, desired capability/outcome, and value; Requirement owns observable required results; Constraint narrows solutions; Contract owns boundary obligations; Outcome owns measurable state change; Scenario owns a concrete behavior example; Invariant owns an always-true condition.
- **DSET-INVARIANT-META-007:** An Outcome is a direct Decision subtype, not an intake item, and owns a measurable change in user, business, operational, or system state without relabeling a delivered output or feature as impact. Requirement evidence proves delivery; Outcome evidence, evaluated against its baseline, target, observation method/source, and window, shows whether delivery had the intended effect.
