...
| `uv run dset new <slug> --package <package-id> --profile <profile> --layer <layer> [--work-area <id> ...]` | Allocate a stable layer-qualified Change ID and create its isolated-workspace manifest for the repository or named Work Areas |
...
1. Create one Change with one stable `DSET-CHANGE-<LAYER>-NNN` ID, readable slug, repository-or-Work-Area target, isolated branch-backed workspace/worktree, and review PR.
...
6. Ordinary Change PRs integrate into `dev`; the release Change carries reviewed `dev` state to protected `main`.
