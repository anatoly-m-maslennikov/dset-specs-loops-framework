...
## DSET-REQUIREMENT-META-009 — Stories add optional actor and value context

When a meaningful actor or stakeholder story exists, accepted project truth may record it with project-wide ID `DSET-STORY-<NNN>` or layer-owned ID `DSET-STORY-<LAYER>-<NNN>`. A Story must capture the actor or stakeholder, desired capability or outcome, value or purpose, and links to the normative Requirements and applicable Scenarios that make the outcome verifiable. A package with no meaningful actor story remains valid without one. Stories are accepted truth, not a fourth intake queue, and never substitute for a normative verifiable Requirement.
