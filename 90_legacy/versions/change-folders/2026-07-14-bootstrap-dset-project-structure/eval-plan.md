...
|---|---|---|
| **DSET-EVAL-PLAN-GOV-001** | Findability | Every reviewer locates the correct path for all five cases without private instructions |
| **DSET-EVAL-PLAN-GOV-002** | Ownership | No reviewer treats `changes/` as accepted truth or `methodology/` as duplicated project truth |
| **DSET-EVAL-PLAN-GOV-003** | Proportionality | Reviewers find the one-package tree sufficient and do not require an empty global layer |
| **DSET-EVAL-PLAN-GOV-004** | Resumability | An agent can identify completed work, the current PR/archive state, and the next task from repository files alone |
