...
`dset/dset_settings.toml`, project-wide records under `dset/project/`, Version
...
their original fields and prose and are not rewritten.
...
each semantic layer one writable fragment under `dset/<layer>/`; connect
fragments with stable
...
| Accepted domain and behavioral specs | layer-owned evergreen Specification files under `dset/<layer>/` |
...
| Implementation plan and tasks | active `dset/versions/changes/<stable-change-id>/` |
